...
type Filter = 'all' | 'urgent' | 'dated' | 'done'
...
function Tasks({ items, members, setItems, onError }: { items: Todo[]; members: Dashboard['members']; setItems: (v: Todo[]) => void; onError: (v: string) => void }) {
  ...
  const [doneItems, setDoneItems] = useState<Todo[] | null>(null)
  const visible = items.filter(item => filter === 'urgent' ? item.priority === 'high' : filter === 'dated' ? Boolean(item.due_at) : true)
  const showDone = async () => { setFilter('done'); if (doneItems === null) { try { const all = await api.tasks(true); setDoneItems(all.filter(x => x.done)) } catch (err) { onError(String(err)) } } }
  ...
  const done = async (item: Todo) => { const before = items; setItems(items.filter(x => x.id !== item.id)); try { const updated = await api.updateTask(item.id, { done: 1 }); if (doneItems !== null) setDoneItems([updated, ...doneItems]); hapticSuccess() } catch (err) { setItems(before); onError(String(err)) } }
  const restore = async (item: Todo) => { const before = doneItems || []; setDoneItems(before.filter(x => x.id !== item.id)); try { const updated = await api.updateTask(item.id, { done: 0 }); setItems([updated, ...items]); hapticSuccess() } catch (err) { setDoneItems(before); onError(String(err)) } }
  ...
  return <section className="page"><PageTitle title="משימות" subtitle={`${items.length} פתוחות`} />
    ...
    <div className="segmented"><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>הכול</button><button className={filter === 'urgent' ? 'active' : ''} onClick={() => setFilter('urgent')}>דחוף</button><button className={filter === 'dated' ? 'active' : ''} onClick={() => setFilter('dated')}>עם מועד</button><button className={filter === 'done' ? 'active' : ''} onClick={showDone}>הושלמו</button></div>
    {filter !== 'done' && <Section title="לביצוע">{visible.length ? visible.map(item => <div className="manage-row" key={item.id}><button className="row-main" onClick={() => done(item)}><span className="check-circle"/><span><strong>{item.title}</strong><small>{item.due_at ? dateText(item.due_at) : item.priority === 'high' ? 'עדיפות גבוהה' : 'ללא מועד'}</small></span>{item.priority === 'high' && <i className="priority">דחוף</i>}</button><button className="more-button" onClick={() => setEditing({ ...item })}>•••</button></div>) : <Empty text="אין משימות בפילטר הזה"/>}</Section>}
    {filter === 'done' && <Section title="הושלמו">{doneItems === null ? <Empty text="טוען…"/> : doneItems.length ? doneItems.map(item => <div className="manage-row task-completed" key={item.id}><div className="row-main"><span className="check-circle checked">✓</span><span><strong>{item.title}</strong><small>{item.done_at ? `הושלמה ${new Intl.DateTimeFormat('he-IL', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date(item.done_at * 1000))}` : 'הושלמה'}</small></span></div><button className="restore-button" onClick={() => restore(item)}>↩ החזרה</button></div>) : <Empty text="עוד לא הושלמו משימות"/>}</Section>}
