...
function App() {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      const [home, shop, taskRows, projectRows, eventRows, activityRows] = await Promise.all([
      ...
      setCalendarStatus(home.calendar_status || { configured: false })
  ...
  const refresh = async () => {
    try { await load() } catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)) }
  }
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="top-actions"><button className={refreshing ? 'icon-button spinning' : 'icon-button'} onClick={refresh} aria-label="רענון">↻</button><div className="avatar" aria-label={userName || 'משתמש'}>{(userName || 'ב').slice(0, 1)}</div></div>
    </header>
    ...
    <nav className="bottom-nav" aria-label="ניווט ראשי">
      ...
      <NavButton active={tab === 'tasks'} icon="✓" label="עבודה" count={tasks.filter(item => !['completed', 'cancelled'].includes(item.status)).length} onClick={() => navigate('tasks')} />
...
function Home({ dashboard, onNavigate }: { dashboard: Dashboard; onNavigate: (tab: Tab) => void }) {
  ...
  const urgent = dashboard.todos.filter(item => item.priority === 'high' && !['completed', 'cancelled'].includes(item.status)).length
  const blocked = dashboard.todos.filter(item => item.blocked).length
  return <section className="page home-page">
    <div className="hero-card"><span className="eyebrow">{greeting}</span><h2>מה צריך לקדם היום?</h2><p>{dashboard.counts.projects || 0} פרויקטים, {dashboard.counts.todos} משימות ו־{dashboard.counts.events} אירועים ביומן.</p><div className="hero-actions"><button onClick={() => onNavigate('tasks')}>＋ משימה או פרויקט</button><button onClick={() => onNavigate('events')}>＋ אירוע</button></div></div>
    ...
    {(urgent > 0 || blocked > 0) && <div className="attention-card"><span>⚡</span><div><strong>{urgent} דחופות · {blocked} חסומות</strong><small>כדאי לפתוח את אזור העבודה ולבחור את הצעד הבא.</small></div><button onClick={() => onNavigate('tasks')}>הצג</button></div>}
    <Section title="פרויקטים בתנועה" action="כל העבודה" onAction={() => onNavigate('tasks')}>{dashboard.projects?.length ? dashboard.projects.slice(0, 3).map(project => <div className="timeline-row" key={project.id}><div className="timeline-dot"/><div><strong>{project.name}</strong><small>{project.progress}% · {project.task_count} משימות</small></div></div>) : <Empty text="אין פרויקטים פעילים"/>}</Section>
    <Section title="בקרוב" action="כל האירועים" onAction={() => onNavigate('events')}>{dashboard.events.length ? dashboard.events.slice(0, 3).map(event => <div className="timeline-row" key={event.id}><div className="timeline-dot"/><div><strong>{event.title}</strong><small>{dateText(event.start_at)}–{new Date(event.end_at).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })}</small></div></div>) : <Empty text="אין אירועים קרובים"/>}</Section>
    <Section title="שינויים אחרונים">{dashboard.activity.length ? dashboard.activity.slice(0, 5).map(item => <ActivityRow key={item.id} item={item}/>) : <Empty text="כאן יופיעו עדכונים מבני הבית"/>}</Section>
  </section>
}

function Shopping({ items, setItems, onError }: { items: ShoppingItem[]; setItems: (value: ShoppingItem[]) => void; onError: (value: string) => void }) {
  const [value, setValue] = useState('')
  const [qty, setQty] = useState('1')
  const [category, setCategory] = useState('')
  const [query, setQuery] = useState('')
  const filtered = items.filter(item => `${item.item} ${item.category}`.toLowerCase().includes(query.toLowerCase()))
  const grouped = useMemo(() => Object.entries(filtered.reduce<Record<string, ShoppingItem[]>>((result, item) => { const key = item.category || 'כללי'; (result[key] ||= []).push(item); return result }, {})), [filtered])
  const add = async (event: FormEvent) => { event.preventDefault(); if (!value.trim()) return; try { const item = await api.addShopping(value.trim(), qty, category.trim()); setItems([item, ...items]); setValue(''); setQty('1'); setCategory(''); hapticSuccess() } catch (caught) { onError(String(caught)) } }
  const done = async (item: ShoppingItem) => { const before = items; setItems(items.filter(row => row.id !== item.id)); try { await api.updateShopping(item.id, { done: 1 }); hapticSuccess() } catch (caught) { setItems(before); onError(String(caught)) } }
  const remove = async (item: ShoppingItem) => { if (!confirm(`למחוק את ${item.item}?`)) return; const before = items; setItems(items.filter(row => row.id !== item.id)); try { await api.deleteShopping(item.id) } catch (caught) { setItems(before); onError(String(caught)) } }
  return <section className="page"><div className="page-title"><div><h2>רשימת קניות</h2><p>{items.length} פריטים פתוחים</p></div></div><form className="stack-form" onSubmit={add}><input aria-label="פריט חדש" value={value} onChange={event => setValue(event.target.value)} placeholder="מה חסר בבית?"/><div className="form-row"><input aria-label="כמות" value={qty} onChange={event => setQty(event.target.value)} placeholder="כמות"/><input aria-label="קטגוריה" value={category} onChange={event => setCategory(event.target.value)} placeholder="קטגוריה"/></div><button className="primary-button">הוספה לרשימה</button></form><input className="search-input" value={query} onChange={event => setQuery(event.target.value)} placeholder="חיפוש ברשימה…"/>{grouped.length ? grouped.map(([group, rows]) => <Section key={group} title={group}>{rows.map(item => <div className="manage-row" key={item.id}><button className="row-main" onClick={() => done(item)}><span className="check-circle"/><span><strong>{item.item}</strong><small>כמות: {item.qty}</small></span></button><button className="danger-icon" onClick={() => remove(item)} aria-label={`מחיקת ${item.item}`}>×</button></div>)}</Section>) : <Empty text="לא נמצאו פריטים"/>}</section>
}

function Settings({ household, setHousehold, activity, onError }: { household: Household; setHousehold: (value: Household) => void; activity: Activity[]; onError: (value: string) => void }) {
  const [name, setName] = useState(household.name)
  const save = async (event: FormEvent) => { event.preventDefault(); try { const updated = await api.updateHousehold({ name }); setHousehold(updated); hapticSuccess() } catch (caught) { onError(String(caught)) } }
