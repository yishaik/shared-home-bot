...
export async function authenticate(initData: string) {
  const result = await request<{ token: string; user: { id: number; name: string }; household: Household }>('/api/auth/telegram', { method: 'POST', body: JSON.stringify({ init_data: initData }) })
  token = result.token
