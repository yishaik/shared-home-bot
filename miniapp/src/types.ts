...
export type TaskRelationship = {
  ...
  relationship_type: 'blocks' | 'related' | 'follows' | 'duplicates'
  source_title?: string
  target_title?: string
}
...
export type TaskCalendarBlock = {
  ...
  block_type: 'work' | 'appointment' | 'review' | 'focus'
  start_at: string
  end_at: string
  location: string
  sync_status: string
...
export type TaskResource = {
  ...
  mime_type: string
  web_url: string
  relationship: 'attachment' | 'working_doc' | 'source' | 'output'
  created_at: number
}
...
export type Todo = {
  ...
  blocked: boolean
  blockers: Array<{ id: number; title: string; status: TaskStatus }>
  relationships: TaskRelationship[]
  calendar_blocks: TaskCalendarBlock[]
...
export type Dashboard = {
  household: Household
  members: Member[]
  counts: { todos: number; shopping: number; events: number; projects: number }
  todos: Todo[]
