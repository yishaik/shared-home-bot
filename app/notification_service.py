...
class NotificationService:
    ...
    async def enqueue_task(self, user_id: int | None, task: dict[str, Any], *, kind: str = "task_assigned") -> None:
        ...
        payload = {key: task.get(key) for key in ("title", "project_name", "due_at", "priority", "status")}
        now = time.time()
        await self.store.db.execute(
            """INSERT INTO telegram_notifications(household_id, user_id, kind, entity_id,
                   payload_json, available_at, created_at) VALUES(?,?,?,?,?,?,?)""",
            (self.store.household_id, int(user_id), kind, str(task.get("id", "")),
    ...
    async def _fail(self, row: dict[str, Any], error: str, *, retry: bool) -> None:
        ...
        available_at = time.time() + min(3600, 30 * (2 ** attempts)) if status == "pending" else float(row.get("available_at") or time.time())
        await self.store.db.execute(
            "UPDATE telegram_notifications SET status=?, available_at=?, attempts=?, last_error=? WHERE id=?",
