...
def build_api_router(settings: Settings, store: Store, service: HomeService) -> APIRouter:
    ...
    async def create_task_calendar_block(
        todo_id: int,
        body: TaskCalendarBlockCreate,
        actor: Actor = Depends(current_actor),
    ) -> dict[str, Any]:
        try:
    ...
    async def delete_task_calendar_block(
        todo_id: int,
        block_id: int,
        actor: Actor = Depends(current_actor),
    ) -> None:
    ...
    async def add_task_resource_link(
        todo_id: int,
        body: TaskResourceLinkCreate,
        actor: Actor = Depends(current_actor),
    ) -> dict[str, Any]:
        try:
