...
class MemberService:
    """Telegram-backed household identity with private identifiers kept server-side."""
    ...
    async def ensure_schema(self) -> None:
        ...
        for name, definition in additions.items():
            if name not in columns:
                await self.store.db.execute(f"ALTER TABLE household_members ADD COLUMN {name} {definition}")
        ...
        *,
        ...
        await self.store.db.execute(
            """UPDATE household_members
               SET display_name=?, username=?, private_chat_id=COALESCE(?, private_chat_id),
                   bot_started_at=CASE WHEN ?=1 THEN COALESCE(bot_started_at, ?) ELSE bot_started_at END,
                   last_seen_at=?, is_active=1
               WHERE household_id=? AND telegram_user_id=?""",
            ((display_name or "").strip(), (username or "").strip(), private_chat_id,
             int(started), now, now, self.store.household_id, int(user_id)),
        )
        await self.store.db.commit()
    ...
    async def list_active(self) -> list[dict[str, Any]]:
        ...
        rows = await (await self.store.db.execute(
            """SELECT telegram_user_id, display_name, username, role, notification_mode,
    ...
    async def get(self, user_id: int) -> dict[str, Any] | None:
        ...
        row = await (await self.store.db.execute(
            """SELECT telegram_user_id, display_name, username, role, notification_mode,
                      bot_started_at, last_seen_at, private_chat_id, google_email
               FROM household_members
               WHERE household_id=? AND telegram_user_id=?""",
