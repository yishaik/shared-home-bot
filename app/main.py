...
from app.smart_inbox_api import build_smart_inbox_router
...
def create_app() -> FastAPI:
    ...
    inbox = SmartInboxService(settings, store, service)
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            log.exception(
                "memory engine attach failed — continuing with lexical fallback"
            )
        ...
        if settings.resolved_mini_app_url:
            await tg_app.bot.set_chat_menu_button(
                menu_button=MenuButtonWebApp(
                    text="הבית",
                    web_app=WebAppInfo(url=settings.resolved_mini_app_url),
                )
            )
        ...
        private_commands = [
            ...
            BotCommand("todos", "משימות ופרויקטים"),
    ...
    api = FastAPI(
        title="Shared Home Bot",
        version="3.2.0",
        lifespan=lifespan,
    )
    api.include_router(build_api_router(settings, store, service))
    api.include_router(build_smart_inbox_router(settings, store, inbox))
    ...
    async def mini_app(path: str = ""):
        ...
        if (
            ...
            and frontend_dir.resolve() in requested.parents
        ):
    ...
    def _setup_ok(secret: str) -> bool:
        return (
            bool(settings.google_oauth_setup_secret)
            and secret == settings.google_oauth_setup_secret
        )
    ...
    async def google_oauth_start(secret: str = ""):
        ...
        if not settings.google_client_id or not settings.resolved_public_url:
            return JSONResponse(
                {"detail": "GOOGLE_CLIENT_ID / public URL not set"},
                status_code=400,
            )
    ...
    async def google_oauth_callback(
        ...
        if error or not code:
            return JSONResponse(
                {"detail": f"oauth error: {error or 'missing code'}"},
                status_code=400,
            )
        import httpx
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(
                ...
                data={
                    ...
                    "redirect_uri": (
                        f"{settings.resolved_public_url}/google/oauth/callback"
                    ),
            ...
            return JSONResponse(
                {
                    "detail": (
                        "no refresh_token "
                        "(re-consent with prompt=consent)"
                    ),
                    "response": data,
                },
                status_code=400,
        ...
        return Response(
            content=(
                ...
                "<h2>✅ Google מחובר</h2>"
                "<p>אפשר לסגור את הדף. Shared Home Bot ישלים מכאן.</p>"
                "</body></html>"
            ),
    ...
    async def google_oauth_token(secret: str = ""):
        if not _setup_ok(secret):
            return Response(status_code=404)
        return {
            "refresh_token": await store.get_setting(
